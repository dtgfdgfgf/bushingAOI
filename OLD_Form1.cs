...
namespace peilin
{
    public partial class Form1 : Form
    {
        ...
        */
        ...
        private System.Threading.Timer _plcMonitorTimer;
        ...
        private void MonitorPLCValues(object state)
        {
            ...
            try
            {
                lock (_monitorLock)
                {
                    ...
                    Log.Debug($"[PLC監控 {DateTime.Now:HH:mm:ss.fff}] D803(OK1)={d803}, D805(OK2)={d805}");
                }
            }
            catch (Exception ex)
            {
                    ...
                    if (app.DetectMode == 0)
                    {
                        try
                        {
                            ...
                            else if (camID == 3)
                            {
                                ...
                                int tries = 3;               // 最小更動：輕量重讀次數
                                int sleepMs = 5;             // 每次等待毫秒數
                                for (int i = 0; i < tries; i++)
                                {
                                    if (app.lastD98Accepted != d98) break; // 已前進，放行
                                    Thread.Sleep(sleepMs);
                                    d98 = PLC_CheckD(98);
                                }
                                if (app.lastD98Accepted == d98)
                                {
                                    // 仍未前進，視為站4誤觸：丟棄此次影像，不遞增 stop3，不入佇列
                                    int s2 = app.counter["stop2"];
                                    int s3 = app.counter["stop3"];
                                    Log.Warning($"站4誤觸被丟棄：D98 未前進(={d98}) stop2={s2}, stop3={s3}");
                                    return;
                                }
                                // 記錄本次已受理的 D98，用於下一次放行判斷（處理 0~14 的循環）
                                app.lastD98Accepted = d98;
                                */
                                // 第四站拍照時，記錄時間戳
        ...
        private List<(string name, float threshold, int? yn)>
        ...
        private bool PassesOutscFilter(DetectionResult defect, int stop)
        {
            ...
            // 條件1: 長寬比為4:1以上
            if (aspectRatio >= int.Parse(app.param[$"outscAspectRatio_{stop}"]))
            {
                return true;
            }

            // 條件2: 長邊100以上且面積5000以上
            if (longerSide >= int.Parse(app.param[$"outscLongerSide_{stop}"]) && area >= int.Parse(app.param[$"outscArea_{stop}"]))
            {
                return true;
            }
        ...
        public bool CheckWhitePixelRatio(Mat image, int stop)
        {
            try
            {
                ...
                if (double.TryParse(app.param[$"white_{stop}"], out standardRatio))
                {
                    ...
                    // 判斷是否在允許範圍內
                    bool a = ratio < standardRatio + 2;
                    bool b = ratio > standardRatio - 2;
                    bool isValid = (a && b);
        ...
        private void UpdateButtonStates()
        {
            BeginInvoke(new Action(() =>
            {
                switch (app.currentState)
                {
                    case app.SystemState.Stopped:
                        button1.Enabled = true;   // 可以開始檢測
                        button2.Enabled = false;  // 不能停止（沒在運行）
                        button47.Enabled = false; // 不能更新計數（沒停止檢測）
                        button17.Enabled = false; // 不能復歸（沒需要）
...
public class ResultManager
{
    ...
    #region 比對D98 D99 以及M值
    // 依 D98/D99 計算 OK1/OK2 應該對應的 M 線圈（處理 1~15 與環狀）
    private static int ExpectedOkMFromD(int dVal, bool isOk1)
    {
        if (dVal <= 0) return -1; // PLC 尚未刷新或異常
        int idx = ((dVal - 1) % 15 + 15) % 15; // 安全環狀索引 0..14
        return isOk1 ? (3000 + idx) : (4500 + idx);
    }

    // 由 GitHub Copilot 產生
    // 記錄「樣品ID、D98/D99、實際脈衝M、依 D98/D99 推算之預期M」
    private static void LogPushMapping(int sampleId, bool isNG, bool isNull, int sigM, string lane)
    {
        // 只要在送脈衝前讀一次即可（時序上最接近實際動作）
        int d98 = Form1.PLC_CheckD(98);
        int d99 = Form1.PLC_CheckD(99);

        if (isNull)
        {
            Log.Information($"[Push] sampleId={sampleId} -> NULL, pulseM=M{sigM} (D98={d98}, D99={d99})");
            return;
        }
        if (isNG)
        {
            Log.Information($"[Push] sampleId={sampleId} -> NG, pulseM=M{sigM} (D98={d98}, D99={d99})");
            return;
        }

        bool isOk1 = string.Equals(lane, "OK1", StringComparison.OrdinalIgnoreCase);
        int expectedM = ExpectedOkMFromD(isOk1 ? d98 : d99, isOk1);

        if (expectedM < 0)
        {
            Log.Warning($"[Push] sampleId={sampleId} -> {lane}, pulseM=M{sigM}, D98={d98}, D99={d99} (PLC尚未刷新到有效值)");
            return;
        }

        if (expectedM != sigM)
        {
            Log.Warning($"[Push] sampleId={sampleId} -> {lane}, D98={d98}, D99={d99}, pulseM=M{sigM}, expectedM=M{expectedM} (MISMATCH)");
        }
        else
        {
            Log.Information($"[Push] sampleId={sampleId} -> {lane}, D98={d98}, D99={d99}, pulseM=M{sigM} == expectedM=M{expectedM}");
        }
    }
    ...
    private void CalculateAndSendPLCSignal(SampleResult sampleResult, bool isNG, bool isNull)
    {
        ...
        try
        {
            ...
            /*
            // 在真正送 M 前記錄一次映射（同時監控 D98/D99 與 sampleId）
            LogPushMapping(
                sampleId,
                isNG,
                isNull,
                sigM,
                isNull ? "NULL" : isNG ? "NG" : ResultManager.activeOkCounter
            );
            */
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (ResultManager.activeOkCounter == "OK1")
                    {
                        Log.Debug($"樣品 {sampleId} 送出時間記錄: {sendTime.ToString("HH:mm:ss.fff")} ms, OK1");
                        Form1.PLC_SetM(sigM, true, true);
                        Form1.PLC_SetM(sigM, false, true);
                        // ✅ PLC 信號成功後立即寫入 sampleID
                        WriteSampleToDefectCounts(sampleId, sendTime);
                        plcSignalSuccess = true; // 標記 PLC 信號發送成功
                    }
                    else
                    {
                        Log.Debug($"樣品 {sampleId} 送出時間記錄: {sendTime.ToString("HH:mm:ss.fff")} ms, OK2");
                        Form1.PLC_SetM(sigM, true, true);
                        Form1.PLC_SetM(sigM, false, true);
                        // ✅ PLC 信號成功後立即寫入 sampleID
                        WriteSampleToDefectCounts(sampleId, sendTime);
                    ...
                    */
                    ...
                    try
                    {
                        if (ResultManager.activeOkCounter == "OK1")
                        {
                            ...
                            // 統一Log格式 - OK1送訊號
                            Log.Information($"[樣品 {sampleId}] 拍攝:{photoTime:HH:mm:ss.fff} | 送訊號:{sendTime:HH:mm:ss.fff} | 結果:OK1");
                        }
                        else // OK2
                        {
                            Form1.PLC_SetM(sigM, true, true);
                            ...
                            // 統一Log格式 - OK2送訊號
                            Log.Information($"[樣品 {sampleId}] 拍攝:{photoTime:HH:mm:ss.fff} | 送訊號:{sendTime:HH:mm:ss.fff} | 結果:OK2");
                        }
                    ...
                    {
                        Log.Error($"PLC 信號發送失敗: {ex.Message}");
                        plcSuccess = false;
                    }
                ...
                if (plcSignalSuccess)
                {
                    ...
                    {
                        string result = isNull ? "NULL" : isNG ? "NG" : ResultManager.activeOkCounter;
                    ...
                    //var d809 = PLC_ModBus.GetValue_32bit(ValueUnit_32bt.D, 809);
                    var d809 = Form1.PLC_CheckD(809);
                    //Log.Information($"樣品 {sampleId} PLC 信號發送成功，應更新計數");
                    //Log.Information($"OK1:{d803}, OK2:{d805}, OK:{d807}, NULL:{d809}, NG:{d801}, {DateTime.Now.ToString("HH:mm:ss.fff")}");
                }
                else
                {
    ...
    public static void IncrementOkCounters(ConcurrentDictionary<string, int> dict, int increment = 1)
    {
        lock (okLock)
        {
            ...
            if (dict[activeOkCounter] >= app.pack) // 已經包裝完一包
            {
                ...
                Log.Warning($"已達 {app.pack}，啟用強制NULL配額：{app.postPackNullQuota} 顆");

                // 重置計數器並切換
                dict[activeOkCounter] = 0;
                activeOkCounter = (activeOkCounter == "OK1") ? "OK2" : "OK1";
            }
            Log.Information($"active:{activeOkCounter} count:{dict[activeOkCounter]}, {DateTime.Now.ToString("HH:mm:ss.fff")}");
        }
    }
