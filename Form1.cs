...
namespace peilin
{
    public partial class Form1 : Form
    {
        ...
        */
        ...
        private static readonly object _disposeTaskLock = new object();
        private static bool _isDisposeTaskRunning = false;
        ...
        // 上一次更新速率的時間
        private DateTime lastRateUpdateTime = DateTime.MinValue;
        // 目前的檢測速率 (樣品/分鐘)
        private double currentDetectionRate = 0;
        // 近一分鐘內檢測的樣品數量
        ...
        static string mydbStr = "data source=" + mydb;
        #endregion
        #endregion
        #region Form
        public Form1()
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            if (!app.start_error)
            {
                ...
                button6.Font = new System.Drawing.Font("新細明體", 26.25F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(136)));
                button6.Location = new System.Drawing.Point(933, 100);
                ...
                if (app.status)
                {
                    if (app.DetectMode == 0)
                    {
                        try
                        {
                            ...
                            else if (camID == 3)
                            {
                                ...
                                //Thread.Sleep(6000); // 1500 2200 5000
                                //PLC_SetD(1, int.Parse(app.param["fourToNG_time_ms_4"]));
                                //PLC_SetM(10, true, true);
                                //PLC_SetM(10, false, true);
                    ...
                    else if (app.DetectMode == 1)
                    {
                        try
                        {
                            if (camID == 0)
                            {
                                Cv2.Circle(Src, new Point(989, 610), 220, Scalar.Red, 3);
                                app.Queue_Bitmap1.Enqueue(new ImagePosition(Src, app.counter["stop" + camID]));
                                app._wh1.Set();
                            }
                            else if (camID == 1)
                            {
                                Cv2.Circle(Src, new Point(943, 627), 500, Scalar.Red, 3);
                                app.Queue_Bitmap2.Enqueue(new ImagePosition(Src, app.counter["stop" + camID]));
                                app._wh2.Set();
                            }
                            else if (camID == 2)
                            {
                                Cv2.Rectangle(Src, new Rect(659, 106, 200, 200), Scalar.Red, 3);
                                Cv2.Rectangle(Src, new Rect(1018, 88, 200, 200), Scalar.Red, 3);
                                Cv2.Rectangle(Src, new Rect(435, 340, 200, 200), Scalar.Red, 3);
                                Cv2.Rectangle(Src, new Rect(1260, 310, 200, 200), Scalar.Red, 3);
                                Cv2.Rectangle(Src, new Rect(453, 680, 200, 200), Scalar.Red, 3);
                                Cv2.Rectangle(Src, new Rect(1280, 680, 200, 200), Scalar.Red, 3);
                                Cv2.Rectangle(Src, new Rect(683, 917, 200, 200), Scalar.Red, 3);
                                Cv2.Rectangle(Src, new Rect(1024, 917, 200, 200), Scalar.Red, 3);
                                app.Queue_Bitmap3.Enqueue(new ImagePosition(Src, app.counter["stop" + camID]));
                                app._wh3.Set();
                            }
                            else if (camID == 3)
                            {
                                app.Queue_Bitmap4.Enqueue(new ImagePosition(Src, app.counter["stop" + camID]));
                                app._wh4.Set();
                            }
                        }
                        catch
                    ...
                    if (app.status && input != null)
                    {
                        if (app.DetectMode == 0)
                        {
                            ...
                            if (原圖ToolStripMenuItem.Checked)
                            {
                                try
                                {
                                    app.Queue_Save.Enqueue(new ImageSave(input.image, @".\image\" + st.ToString("yyyy-MM") + "\\" + st.ToString("MMdd") + "\\" + app.foldername + "\\origin\\" + fname));
                                    app._sv.Set();
                                }
                                catch (Exception ex)
                                {
                            ...
                            PerformanceProfiler.StartMeasure($"{input.count}_ProcessSample1");
                            ...
                            // 檢查白色像素占比 (NULL)
                            /*
                            PerformanceProfiler.StartMeasure($"{input.count}_CheckWhitePixelRatio1");
                            bool isValidImage = CheckWhitePixelRatio(input.image, input.stop);
                            PerformanceProfiler.StopMeasure($"{input.count}_CheckWhitePixelRatio1");
                            if (!isValidImage)
                            {
                                // 顯示檢測結果
                            ...
                            */
                            ...
                            PerformanceProfiler.StartMeasure($"{input.count}_findGapWidth1");
                            (bool gapIsNG, Mat gapResult, List<Point> non) = findGapWidth(input.image, input.stop);
                            PerformanceProfiler.StopMeasure($"{input.count}_findGapWidth1");
                            ...
                            #region 找NROI
                            PerformanceProfiler.StartMeasure($"{input.count}_DetectAndExtractROI1");
                            Mat roi = DetectAndExtractROI(input.image, input.stop, input.count);
                            PerformanceProfiler.StopMeasure($"{input.count}_DetectAndExtractROI1");
                                    ...
                                    foreach (var defect in chamferDetection.detections)
                                    {
                                        ...
                                        if (performNonRoiDetection && nonRoiRects.Count > 0)
                                        {
                                            ...
                                            foreach (var nonRoiRect in nonRoiRects)
                                            {
                                                ...
                                                {
                                                    expand_in = int.Parse(app.param[$"expandNROI_in_{input.stop}"]);
                                                    expand_out = int.Parse(app.param[$"expandNROI_out_{input.stop}"]);
                                                }
                                                else if (nonRoiRect.className == "mouth")
                                                {
                                                    expand_in = int.Parse(app.param[$"expandNROI_in_3"]);
                                                    expand_out = int.Parse(app.param[$"expandNROI_out_4"]);
                                                }
                            ...
                            if (defectDetection.detections != null && defectDetection.detections.Count > 0)
                            {
                                ...
                                foreach (var defect in defectDetection.detections)
                                {
                                    ...
                                    if (performNonRoiDetection && nonRoiRects.Count > 0)
                                    {
                                        ...
                                        Mat nonRoiVisImage = input.image.Clone();
                                        foreach (var nonRoiRect in nonRoiRects)
                                        {
                                            double iou = CalculateIoU(defectRect, nonRoiRect.rect);
                            ...
                            PerformanceProfiler.StopMeasure($"{input.count}_ProcessSample1");
                ...
                if (app.Queue_Bitmap2.Count > 0)
                {
                    ...
                    if (app.status && input != null)
                    {
                        if (app.DetectMode == 0)
                        {
                            ...
                            string sampleIdentifier = $"Sample_{input.count}";
                            PerformanceProfiler.StartMeasure($"{input.count}_ProcessSample2");
                            Log.Debug($"樣品 {input.count} 第2站處理開始時間: {DateTime.Now.ToString("HH:mm:ss.fff")}");

                            #region 是否為有效取像，只在這裡做!
                            // 0801: 需新增區分真NULL/變形功能

                            PerformanceProfiler.StartMeasure($"{input.count}_CheckObjectPosition2");
                            //bool isValidImage = CheckWhitePixelRatio(input.image, input.stop);
                            (bool isObjectNG, Mat CheckObjectResultMat) = CheckObjectPosition(input.image, input.stop);
                            PerformanceProfiler.StopMeasure($"{input.count}_CheckObjectPosition2");

                            if (isObjectNG)
                            {
                                // 顯示檢測結果
                                showResultMat(CheckObjectResultMat, input.stop);
                            ...
                            */
                            PerformanceProfiler.StartMeasure($"{input.count}_findGapWidth2");
                            (bool gapIsNG, Mat gapResult, List<Point> gapPositions) = findGapWidth(input.image, input.stop);
                            PerformanceProfiler.StopMeasure($"{input.count}_findGapWidth2");
                            ...
                            #region 找NROI
                            PerformanceProfiler.StartMeasure($"{input.count}_DetectAndExtractROI2");
                            Mat roi = DetectAndExtractROI(input.image, input.stop, input.count);
                            PerformanceProfiler.StopMeasure($"{input.count}_DetectAndExtractROI2");
                            ...
                            PerformanceProfiler.StopMeasure($"{input.count}_ProcessSample2");
                            //PerformanceProfiler.FlushToDisk();
                ...
                if (app.Queue_Bitmap3.Count > 0)
                {
                    ...
                    if (app.status && input != null)
                    {
                        if (app.DetectMode == 0)
                        {
                            ...
                            string sampleIdentifier = $"Sample_{input.count}";
                            PerformanceProfiler.StartMeasure($"{input.count}_ProcessSample3");
                            ...
                            */
                            ...
                            PerformanceProfiler.StartMeasure($"{input.count}_DetectAndExtractROI3");
                            Mat roi = DetectAndExtractROI(input.image, input.stop, input.count);
                            PerformanceProfiler.StopMeasure($"{input.count}_DetectAndExtractROI3");                           
                            ...
                            // 對 outsc 瑕疵進行過殺降低處理
                            List<DetectionResult> processedDefects = ApplyOutscOverkillReduction(validDefects);
                            ...
                            // 繪製檢測結果
                            Mat resultImage = _yoloDetection.DrawDetectionResults(input.image.Clone(), new DetectionResponse { detections = processedDefects }, threshold);
                            ...
                            // 使用現有的 showResultMat 方法顯示結果
                            showResultMat(resultImage.Clone(), input.stop);
                            ...
                            PerformanceProfiler.StopMeasure($"{input.count}_ProcessSample3");
                ...
                if (app.Queue_Bitmap4.Count > 0)
                {
                    ...
                    if (app.status && input != null)
                    {
                        if (app.DetectMode == 0)
                        {
                            ...
                            // 記錄樣本ID，方便追蹤
                            string sampleIdentifier = $"Sample_{input.count}";
                            PerformanceProfiler.StartMeasure($"{input.count}_ProcessSample4");

                            #region 本站不測是否為有效圖像、變形
                            /*
                            // 檢查白色像素占比 (NULL)
                            PerformanceProfiler.StartMeasure($"{input.count}_CheckWhitePixelRatio4");
                            bool isValidImage = CheckWhitePixelRatio(input.image, input.stop);
                            PerformanceProfiler.StopMeasure($"{input.count}_CheckWhitePixelRatio4");
                            ...
                            */
                            ...
                            #region 找NROI
                            PerformanceProfiler.StartMeasure($"{input.count}_DetectAndExtractROI4");
                            Mat roi = DetectAndExtractROI(input.image, input.stop, input.count);
                            PerformanceProfiler.StopMeasure($"{input.count}_DetectAndExtractROI4");
                            ...
                            */
                            ...
                            #endregion
                            PerformanceProfiler.StopMeasure($"{input.count}_ProcessSample4");
                            //var totalTime = PerformanceProfiler.StopMeasure("AI_singleProcess4");
                            PerformanceProfiler.FlushToDisk();
            ...
            using (var db = new MydbDB())
            {
                ...
                if (q.Count() > 0)
                {
                    foreach (var item in q)
                    {
                        ...
                        if (item.Count < 0)
                        {
                            lbAdd($"發現負數計數值: {item.Name}={item.Count}，已自動轉換為0", "warning", "");
        ...
        void updateLabel()
        {
            ...
            {              
                ...
                PLC_ModBus.CheckValue(1, ValueUnit.D, 801, 8, false, handle1);
                handle1.WaitOne();
                ...
                //var d813 = PLC_ModBus.GetValue_32bit(ValueUnit_32bt.D, 809);
        ...
        async void setNet()
        {
            ...
            try
            {
                ...
                bool has_NROI_InnerModel = File.Exists(inner_NROI_ModelPath);
                bool has_NROI_OuterModel = File.Exists(outer_NROI_ModelPath);
                ...
                {
                    lbAdd($"[setNet] 找不到內環模型文件: {innerModelPath}", "warn", "");
                }
                ...
                {
                    lbAdd($"[setNet] 找不到外環模型文件: {outerModelPath}", "warn", "");
                }
                // 初始化內環非ROI伺服器和載入模型
                if (has_NROI_InnerModel)
                {
                ...
                {
                    lbAdd($"[setNet] 找不到內環非ROI模型文件: {inner_NROI_ModelPath}", "warn", "");
                }

                // 初始化外環非ROI伺服器和載入模型
                if (has_NROI_OuterModel)
                {
                ...
                {
                    lbAdd($"[setNet] 找不到外環非ROI模型文件: {outer_NROI_ModelPath}", "warn", "");
                }
                ...
                {
                    lbAdd($"[setNet] 找不到倒角模型文件: {chamfer_ModelPath}", "warn", "");
                }
                ...
                {
                    lbAdd($"[setNet] 找不到站1模型文件: {station1_ModelPath}", "warn", "");
                }
                ...
                {
                    lbAdd($"[setNet] 找不到站2模型文件: {station2_ModelPath}", "warn", "");
                }
                ...
                if (has_NROI_InnerModel && !string.IsNullOrEmpty(app.produce_inner_NROI_ServerUrl))
                {
                    _yoloDetection.StartContinuousWarmup(app.produce_inner_NROI_ServerUrl, 2);
                }

                if (has_NROI_OuterModel && !string.IsNullOrEmpty(app.produce_outer_NROI_ServerUrl))
                {
                        ...
                        if (result == DialogResult.OK) //若使用者按下「OK」，表示同意正式開始檢測程序。
                        {
                            ...
                            if (lastCounts == null || lastCounts.Count == 0)
                            {
                                using (var db = new MydbDB())
                                {
                                    ...
                                    // 為 OK/NG/NULL 創建記錄
                                    string[] generalTypes = new[] { "OK", "NG", "NULL" };
        ...
        private void 關閉程式ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (exportResult == DialogResult.Yes)
                {
                    ...
                    if (success)
                    {
                        lbAdd($"瑕疵統計已成功匯出至: {filePath}", "info", "");
                    }
                    else
                    {
                        lbAdd("匯出瑕疵統計失敗", "warning", "");
                    }
                }
            }
            catch (Exception ex)
            {
                lbAdd("匯出瑕疵統計時發生錯誤", "error", ex.Message);
            }
        ...
        void clear()    //  全清
        {
            ...
            app.detect_result_check.Clear();
            app.counter.Clear();
            var counterType = new[] { "OK1", "OK2", "OK", "OK_TOTAL", "NG", "NULL", "stop0", "stop1", "stop2", "stop3", "blowOK1", "blowOK2", "blowNG" };
            foreach (var s in counterType)
            {
                app.counter.Add(s, 0);
            }
                                ...
                                if (!textBox4.Enabled)
                                {
                                    if (!textBox5.Enabled)
                                    {
                                        if (!comboBox2.Enabled)
                                        {
                                            ...
                                            // 新增：檢查出料卡料時間設定
                                            if (CheckDeliveryTimeSettings())
                                            {
                                                switchButton(false);
                                            }
                                            else
                                            {
                                                MessageBox.Show("尚未設定出料卡料時間，請先設定D8(卡料倒數時間)和D9(沒料倒數時間)。",
                                                               "出料卡料時間未設定",
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            //PLC_SetM(401, true); //重設箱子狀態
            bool writeSuccess = DefectCountManager.PerformPeriodicWrite();
                ...
                if (app.status)
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            //var handle1 = new ManualResetEvent(false);
                            //PLC_ModBus.CheckPoint(1, BoolUnit.M, 81, 5, false, handle1);
                            //handle1.WaitOne();

                            if (!PLC_CheckX(0) || PLC_CheckM(27) || PLC_CheckM(21)) //急停訊號
                            {
                                ...
                                this.BeginInvoke(new Action(() =>
                                {
                                    if (alertForm == null || alertForm.IsDisposed)
                                    {
                                        ...
                                        app.plc_stop = true;
                                        alertForm = new alert();
                                        alertForm.FormClosed += (s, e) => alertForm = null;
                                        alertForm.ShowDialog();
                                    }
                                    // else: 已有 alert 視窗，不再重複開啟
                                }));
                                */
                            }
        ...
        private List<(string name, float threshold, int? yn, int? imageSize)>

        GetDefectInfos(string produceNo, int stopVal)
        {
            using (var db = new MydbDB())
            {
                var q = from c in db.DefectChecks
                        where c.Type == produceNo
                              && c.Stop == stopVal
                        select new
                        {
                            c.Name,
                ...
                var list = new List<(string, float, int?, int?)>();
                foreach (var item in q)
                {
                    list.Add((
                        item.Name,
                        float.Parse(item.Threshold),  // double->float
                        item.Yn,
                        item.Image_Size.HasValue ? item.Image_Size.Value : 480
                    ));
                }
                return list;
            }
        ...
        /// 執行霍夫圓檢測，找出圖像中的內外圓
        /// </summary>
        /// <param name="inputImage">輸入圖像</param>
        /// <param name="stop">站點編號，用於獲取對應參數</param>
        /// <returns>包含外圓陣列和內圓陣列的元組</returns>
            ...
            try
            {
                ...
                try
                {
                    ...
                    */
                    ...
                    if (stop == 1 || stop == 2)
                    {
                        int chamferCenterX = int.Parse(app.param[$"chamfer_center_x_{stop}"]);
                        int chamferCenterY = int.Parse(app.param[$"chamfer_center_y_{stop}"]);
                        int chamferRadius = int.Parse(app.param[$"chamfer_radius_{stop}"]);
        ...
        public (bool hasDefect, Mat resultImg) DetectScratch(Mat src, int stop)
        {
            ...
            int deepenThresh = int.Parse(app.param[$"deepenThresh_{stop}"]);
            int minScratch = int.Parse(app.param[$"minScratch_{stop}"]);
            int maxScratch = int.Parse(app.param[$"maxScratch_{stop}"]);
            try
            {
                // 1) 建立 darkenImage
                Mat darkenImage = AdjustContrast(src, deepenContrast, deepenBrightness);
        ...
        public (bool isNG, Mat img, List<Point> gapPositions) findGapWidth(Mat img, int stop/*, CircleSegment[] outerCircles, CircleSegment[] innerCircles*/)
        {
            ...
            if (minthresh == 0)
            {
                return (false, visualImg, gapPositions); // 0表示不做阈值
            }
            ...
            */
            ...
            {
                ori.Dispose(); // 如果返回 visualImg
                return (true, visualImg, gapPositions);
            }
            else
            {
                /*
                visualImg.Dispose(); // 如果返回 ori
                return (false, ori);
                */
        ...
        {
            if (src == null)
                throw new ArgumentNullException(nameof(src), "输入图像不能为空。");
            if (contrast < -100 || contrast > 100)
                throw new ArgumentOutOfRangeException(nameof(contrast), "对比度应在 -100 到 100 之间。");
            if (brightness < -100 || brightness > 100)
                throw new ArgumentOutOfRangeException(nameof(brightness), "亮度应在 -100 到 100 之间。");
        ...
        /// 檢驗輪廓在二值化圖像中是否為黑色區域
        /// </summary>
        /// <param name="binaryImage">二值化圖像（0=黑色，255=白色）</param>
        /// <param name="contour">要檢驗的輪廓</param>
        /// <param name="blackRatioThreshold">黑色像素比例閾值（預設70%）</param>
        /// <returns>是否主要為黑色</returns>
        private bool IsContourBlackInBinary(Mat binaryImage, Point[] contour, double blackRatioThreshold = 0.7)
        {
            try
            {
                // 創建輪廓的遮罩
                Mat contourMask = new Mat(binaryImage.Size(), MatType.CV_8UC1, Scalar.Black);
                Cv2.FillPoly(contourMask, new Point[][] { contour }, Scalar.White);
                ...
                for (int y = boundingRect.Y; y < boundingRect.Y + boundingRect.Height; y++)
                {
                    ...
                    {
                        // 檢查這個像素是否在輪廓內
                        if (contourMask.Get<byte>(y, x) > 0)
                        {
                            totalPixels++;
                            byte pixelValue = binaryImage.Get<byte>(y, x);

                            // 在二值化圖像中，0表示黑色，255表示白色
                            if (pixelValue == 0)
                            {
                                blackPixels++;
                            }
                ...
                // 調試輸出（可選）
                if (app.param.ContainsKey("debugBlackDotValidation") && app.param["debugBlackDotValidation"] == "1")
                {
                    Console.WriteLine($"輪廓內部像素分析: 總像素={totalPixels}, 黑色像素={blackPixels}, " +
                                    $"黑色比例={blackRatio:P2}, 閾值={blackRatioThreshold:P2}");
                }
            ...
            {
                Log.Error($"檢驗輪廓黑色像素時發生錯誤: {ex.Message}");
                return false;
            }
        }
        ...
        private List<DetectionResult> ApplyOutscOverkillReduction(List<DetectionResult> validDefects)
        {
            // 篩選出outsc瑕疵和其他瑕疵
            ...
            if (outscDefects.Count == 2)
            {
                ...
                {
                    // 分別對每個瑕疵進行篩選
                    filteredOutscDefects.AddRange(outscDefects.Where(defect => PassesOutscFilter(defect)));
                }
            }
        ...
        private bool PassesOutscFilter(DetectionResult defect)
        {
            int width = defect.box[2] - defect.box[0];
            int height = defect.box[3] - defect.box[1];
            int area = width * height;
        ...
        private void button22_Click(object sender, EventArgs e) //待刪
        {
            if (button22.Text == "ON")
            {
                button22.Text = "OFF";
                button22.BackColor = button22.BackColor = Color.FromArgb(255, 128, 128);
                app.param["packsignal"] = "false";
                using (var db = new MydbDB())
                {
                    db.Parameters.Where(p => p.Name == "packsignal").Set(p => p.Value, "false").Update();
                }
                PLC_SetM(26, true);
            }
            else
            {
                button22.Text = "ON";
                button22.BackColor = button22.BackColor = Color.Lime;
                app.param["packsignal"] = "true";
                using (var db = new MydbDB())
                {
                    db.Parameters.Where(p => p.Name == "packsignal").Set(p => p.Value, "true").Update();
                }
                PLC_SetM(26, false);
            }
        }
        ...
        private void button26_Click(object sender, EventArgs e) // 閥門檔板 待改 只顯示就好
        {
        ...
        {
            app.output_result = comboBox8.SelectedIndex;
        }
            ...
            try
            {
                ...
                using (var dialog = new FolderBrowserDialog())
                {
                    ...
                    if (Directory.Exists(@"C:\Users\User\Desktop\peilin2_TEST0303\bin\x64\Release"))
                        dialog.SelectedPath = @"C:\Users\User\Desktop\peilin2_TEST0303\bin\x64\Release\image\2025-05\0522(done)";
        ...
        private async void button37_Click(object sender, EventArgs e)
        {
            ...
            lbAdd($"找到 {sortedFiles.Length} 張測試圖片", "info", "");
            ...
            try
            {
                lbAdd($"開始離線測試模式，從 {testImagesFolder} 讀取圖片", "info", "");
                ...
                foreach (var fileInfo in sortedFiles)
                {
                    ...
                    {
                        lbAdd($"圖片 {Path.GetFileName(fileInfo.FilePath)} 站號 {fileInfo.StationId} 不在有效範圍1-4內，已跳過", "warning", "");
                        continue;
                    }
                    ...
                    if (src.Empty())
                    {
                        lbAdd($"無法讀取圖片: {Path.GetFileName(fileInfo.FilePath)}", "warning", "跳過此圖片");
                        continue;
                    }
                    ...
                    lbAdd($"將圖片 {Path.GetFileName(fileInfo.FilePath)} 送入相機{fileInfo.StationId}處理流程，樣品編號:{fileInfo.SampleId}", "info", "");
                    ...
                    {
                        lbAdd($"已處理 {processedImageCount} 張圖片", "info", "");
                    ...
                    if (!app.status)
                    {
                        lbAdd("測試被使用者中斷", "warning", "");
                        break;
                    }
                }

                // 等待所有隊列處理完畢
                lbAdd("等待所有隊列處理完畢...", "info", "");
                int timeout = 0;
                      ...
                      timeout < 100 && app.status)
                {
                    ...
                    if (timeout % 10 == 0)
                    {
                        lbAdd($"隊列剩餘項目: 相機1={app.Queue_Bitmap1.Count}, 相機2={app.Queue_Bitmap2.Count}, " +
                              $"相機3={app.Queue_Bitmap3.Count}, 相機4={app.Queue_Bitmap4.Count}", "info", "");
                        Application.DoEvents(); // 更新UI
                    }
                }
            ...
            {
                lbAdd("測試過程中發生錯誤", "error", ex.Message);
        ...
        private void button46_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    lbAdd("手動分析圖片時發生錯誤", "error", ex.Message);
                }
            }
        }
        ...
        private void UpdateDetectionRate()
        {
            ...
            // 安全地更新 UI
            BeginInvoke(new Action(() => {
                label13.Text = samplesPerMinute.ToString("F1"); // 每分鐘樣品數，顯示一位小數
        ...
        public bool CheckWhitePixelRatio(Mat image, int stop)
        {
            try
            {
                ...
                if (double.TryParse(app.param[$"white_{stop}"], out standardRatio))
                {
                    ...
                    // 判斷是否在允許範圍內
                    bool a = ratio < standardRatio;
                    bool b = ratio > standardRatio - 3;
                    bool isValid = (a && b);

                    // 記錄日誌
                    //lbAdd($"站點 {stop} 白色像素占比: {ratio:F2}%, 標準值: {standardRatio}%, 允許範圍: [{lowerBound:F2}%, {upperBound:F2}%], 有效性: {(isValid ? "有效" : "無效")}", "Info", "PixelRatioCheck");
                    Console.WriteLine($"站點 {stop} 白色像素占比: {ratio:F2}%, 標準值: {standardRatio}%, 有效性: {(isValid ? "有效" : "無效")}", "Info", "PixelRatioCheck");
                ...
                {
                    // 若參數不存在，默認為有效
                    lbAdd($"站點 {stop} 的白色像素占比參數未設定，默認視為有效圖像", "Warning", "PixelRatioParamMissing");
                    return true;
                }
            }
            catch (Exception ex)
            {
                lbAdd($"檢查白色像素占比時發生錯誤: {ex.Message}", "Error", ex.Message);
        ...
        public (bool hasDefect, Mat resultImg) CheckObjectPosition(Mat image, int stop)
        {
            ...
            try
            {
                ...
                // 2. 計算圖像中心（預期中心點） //預設物體中心!!
                Point imageCenter = new Point(image.Width / 2 + objBiasX, image.Height / 2 + objBiasY);
                ...
                {
                    lbAdd($"站點 {stop} - 未找到適當大小的物體輪廓", "Warning", "輪廓面積應在200000-1000000像素範圍");

                    // 在結果圖上標記預期位置和容許範圍
                    Cv2.Circle(resultImg, imageCenter, 10, new Scalar(0, 0, 255), -1);  // 紅色-圖像中心
                    Cv2.Circle(resultImg, imageCenter, tolerance, new Scalar(0, 255, 255), 2);  // 黃色圈-容許範圍
                ...
                if (moments.M00 == 0)
                {
                    lbAdd($"站點 {stop} - 無法計算物體質心", "Error", "輪廓矩計算失敗");
                ...
                // 8. 計算與圖像中心的偏差
                double offsetX = objectCenter.X - imageCenter.X;
                double offsetY = objectCenter.Y - imageCenter.Y;
                ...
                // 繪製圖像中心點（紅色）
                Cv2.Circle(resultImg, imageCenter, 10, new Scalar(0, 0, 255), -1);
                Cv2.PutText(resultImg, "Image Center",
                           new Point(imageCenter.X + 15, imageCenter.Y - 15), HersheyFonts.HersheySimplex, 0.8, new Scalar(0, 0, 255), 2);
                ...
                // 繪製從物體中心到圖像中心的連線
                Cv2.Line(resultImg, objectCenter, imageCenter, new Scalar(255, 255, 0), 2);

                // 繪製容許範圍圓圈
                Cv2.Circle(resultImg, imageCenter, tolerance,
            ...
            {
                lbAdd($"檢查物體位置時發生錯誤: {ex.Message}", "Error", ex.ToString());
        ...
        private bool CheckChamfer(Mat image, int stop) //倒角AOI
        {
            ...
            if (double.TryParse(app.param[$"chamfer_{stop}"], out standardRatio))
            {
                ...
                // 記錄日誌
                //lbAdd($"站點 {stop} 白色像素占比: {ratio:F2}%, 標準值: {standardRatio}%, 允許範圍: [{lowerBound:F2}%, {upperBound:F2}%], 有效性: {(isValid ? "有效" : "無效")}", "Info", "PixelRatioCheck");
                Console.WriteLine($"站點 {stop} 白色像素占比: {ratio:F2}%, 標準值: {standardRatio}%, 允許範圍: [{lowerBound:F2}%, {upperBound:F2}%], 有效性: {(isValid ? "有效" : "無效")}", "Info", "PixelRatioCheck");
            ...
            using (FolderBrowserDialog folderDialog = new FolderBrowserDialog())
            {
                ...
                if (folderDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    try
                    {
                        ...
                        using (var progressForm = new Form())
                        {
                            ...
                            foreach (string imagePath in imageFiles)
                            {
                                ...
                                {
                                    lbAdd($"無法讀取圖片: {imagePath}", "Error", "ImageReadFailed");
                                    continue;
                                }
                    ...
                    catch (Exception ex)
                    {
                        ...
                        lbAdd($"處理Chamfer檢測時發生錯誤: {ex.Message}", "Error", "ChamferProcessError");
                    }
                }
            }
        }
        ...
        private void CreateHistogram(List<double> ratioValues, string folderPath, int stop)
        {
            ...
            {
                lbAdd($"創建直方圖時發生錯誤: {ex.Message}", "Error", "HistogramError");
            }
        }
        // 統計數據的類別
    ...
    public static class DefectCountManager
    {
        ...
        /// <returns>寫入是否成功</returns>
        public static bool WriteDefectCounts(List<(string Type, string Name, int Count, string LotId)> items)
        {
            ...
            Dictionary<string, int> generalCounts = null)
        {
            try
            {
                ...
                return WriteDefectCounts(itemsToWrite);
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <returns>寫入是否成功</returns>
        public static bool PerformPeriodicWrite() //按停止鈕時寫入
        {
            try
            {
                ...
                */
                ...
                // 寫入所有計數資料
                return WriteAllDefectCounts(app.produce_No, app.LotID, app.dc, generalCounts);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[Error] 定期寫入 DefectCount 失敗: {ex.Message}");
                return false;
            }
        }
    }
...
public class ResultManager
{
    ...
    private void CombineResults(SampleResult sampleResult)
    {
        ...
        if (hasNullResult)
        {
            ...
            // 創建一個特殊的最終結果，表示這是一個 NULL 樣品
            //bool finalIsNG = false; // NULL 不視為 NG

            // 儲存結果 - 特別標記為 NULL 類型
            //SaveFinalResult(sampleResult, false, isNull: true);
        ...
        try
        {
            ...
            if (shouldSaveStations)
            {
                ...
                foreach (var stationResult in sampleResult.StationResults.Values)
                {
                    ...
                    // 在图像上標記文字
                    Mat markedImage = stationResult.FinalMap.Clone();
                    //using (Mat markedImage = stationResult.FinalMap.Clone())
                    {
                        ...
                        // 保存原有路徑圖像
                        app.Queue_Save.Enqueue(new ImageSave(markedImage, savePath));
    ...
    private void CalculateAndSendPLCSignal(SampleResult sampleResult, bool isNG, bool isNull)
    {
        ...
        try
        {
            ...
            {
                left = timeDifference.TotalMilliseconds - (double.Parse(app.param["fourToOK1_time_ms_4"]) - double.Parse(app.param["delay4"]));
            }
            else if (ResultManager.activeOkCounter == "OK2" && isNG == false)
            {
                left = timeDifference.TotalMilliseconds - (double.Parse(app.param["fourToOK2_time_ms_4"]) - double.Parse(app.param["delay4"]));
            }
            else if (isNG == true)
            {
                left = timeDifference.TotalMilliseconds - (double.Parse(app.param["fourToNG_time_ms_4"]) - double.Parse(app.param["delay4"]));
            }
            if (left > 0)
            {
                /*
                ...
                {
                    // 使用負數參數來減少OK計數
                    IncrementOkCounters(counter, -1);
                    Log.Information($"樣品 {sampleId} 因超時，已反向調整OK計數");
                }
                */
                isNull = true;
                timeout = true;                
            ...
            try
            {
                ...
                if (isNull)
                {
                    Log.Debug($"樣品 {sampleId} 送出時間記錄: {resultTime.ToString("HH:mm:ss.fff")} ms");

                    IncrementCounter(counter, "NULL");
                    Form1.PLC_SetM(sigM, true, true);
                    Form1.PLC_SetM(sigM, false, true);
                    plcSignalSuccess = true; // 標記 PLC 信號發送成功              
                }
                else if (isNG)
                {
                    Log.Debug($"樣品 {sampleId} 送出時間記錄: {resultTime.ToString("HH:mm:ss.fff")} ms");
                ...
                if (plcSignalSuccess)
                {
                    ...
                    if (timeout == true)
                    {
                        ...
                        Log.Information($"樣品 {sampleId} 超時，已更新計數");
                    }
                    else
                    {
                        SaveFinalResult(sampleResult, isNG, isNull);
                        Log.Information($"樣品 {sampleId} PLC 信號發送成功，已更新計數");
                    }
                }
                else
                {
            ...
            catch (Exception plcEx)
            {
                ...
                // 可以在這裡增加警報或其他機制通知操作員
                //lbAdd("PLC通訊失敗", "Error", $"無法發送推料信號，請檢查PLC連線: {plcEx.Message}");
    ...
    public static void IncrementOkCounters(ConcurrentDictionary<string, int> dict, int increment = 1)
    {
        lock (okLock)
        {
            ...
            if (dict[activeOkCounter] >= app.pack) // 已經包裝完一包
            {
                ...
                {
                    Form1.PLC_SetM(6, false);
                    Form1.PLC_SetM(7, true);
                }
                else if(activeOkCounter == "OK2")
                {
                    Form1.PLC_SetM(6, true);
                    Form1.PLC_SetM(7, false);
                }
                */
            }
        }
